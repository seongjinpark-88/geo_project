...
def run_kfold():
    ...
    for train_index, test_index in kf.split(data):
        ...
        to_features_svm = classify.TextToFeatures(train_texts)
        ...
        clf3.train(to_features_svm(train_texts), to_labels(train_labels))
        ...
        all_preds = []
        assert len(pred1) == len(pred2) == len(pred3)
        if len(pred1) == len(pred2):
            for i in range(0, np.shape(pred2)[0]):
                all_preds.append([pred1[i], pred2[i], pred3[i]])
            ...
            for i in range(len(all_preds)):
                if 0 in all_preds[i]:
                    ...
                    nb_pred = all_preds[i][0]
                    svm_pred = all_preds[i][1]
                    mlp_pred = all_preds[i][2]

                    holder = Counter(all_preds[i])
                    held = [(v, int(k)) for k, v in holder.items()]
                    held = sorted(held, reverse = True)
                    if len(held) > 1:
                        if held[0][0] == 1:
                            if 3 not in [held[0][1], held[1][1], held[2][1]]:
                                final_pred[i] = mlp_pred
                            else:
                                if mlp_pred == 3 and nb_pred == 3:
                                    final_preds[i] = svm_pred
                                elif mlp_pred == 3 and svm_pred != 3:
                                    final_preds[i] = svm_pred
                                else:
                                    final_preds[i] = mlp_pred

                        elif held[0][0] == 2:
                            if held[0][1] == 3:
                                final_preds[i] == held[1][1]
                            else:
                                final_preds[i] == held[0][1]
                        else: #otherwise, select majority voted
                            final_preds[i] = held[0][1]
                    else: # if all predicted the same
                        final_preds[i] = held[0][1]

            predicted_indices = final_preds
